    ...
    <main className="min-h-screen bg-[var(--canvas)] text-[var(--ink)]">
      ...
      <section className="mx-auto w-full max-w-[1240px] px-5 pb-24 pt-10 sm:px-8 sm:pt-14">
        ...
        <div className={variant === "dropdowns" ? "" : "rounded-2xl bg-[var(--surface)] p-3 shadow-[var(--shadow-panel)]"}>
          ...
          {/* v3 and v4 show selected filters as dashed "[icon] is [value]" chips; v4 has no card, so
              it drops the top border. */}
          {activeChips.length > 0 && (
            <div className={`flex flex-wrap items-center gap-2 ${variant === "dropdowns" ? "mt-3" : "mt-3 border-t border-black/6 pt-3"}`}>
              {variant === "chips" || variant === "dropdowns" ? (
                <>
...
function LocationSearch({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div>
      <SearchBox full value={value} onChange={onChange} placeholder="City, region or country" />
...
// v4 pill: 36px tall, 8px padding, 14px radius, a hairline shadow in place of a border, white fill.
const V4_PILL =
  "inline-flex h-9 shrink-0 items-center gap-1.5 rounded-[14px] bg-white px-2 text-sm font-medium text-[var(--ink)] shadow-[var(--shadow-control)] transition-colors duration-150 hover:bg-[#F5F5FA] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--focus)]";
...
function SearchBox({
  ...
  return (
    <label className={`flex h-9 shrink-0 cursor-text items-center gap-2 rounded-[14px] px-2 transition-colors duration-150 ${shell}`}>
      <IconSearchGlyph />
      <input
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="ms-auto flex items-center gap-2">
        ...
        <SortDropdown value={filters.sort} onChange={(value) => update({ sort: value })} />
...
// Single-select order dropdown (v4), styled like the filter pills. Relevance takes over while a text
// search is active, so this drives the browse order.
const SORT_OPTIONS_UI = [
...
function SortDropdown({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <FilterDropdown Icon={IconSortF} label="Sort" count={0} width="w-44">
      <div className="flex flex-col">
        {SORT_OPTIONS_UI.map((option) => {
          const active = value === option.value || (option.value === "newest" && !value);
          return (
            <button
              key={option.value}
              type="button"
              onClick={() => onChange(option.value)}
              className="flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 text-start text-sm text-[var(--ink)] transition-colors duration-100 hover:bg-[var(--control-hover)]"
            >
              {option.label}
              {active && (
                <svg viewBox="0 0 16 16" aria-hidden="true" className="size-4 shrink-0 text-[var(--accent-strong)]" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3.5 8.5 6.5 11.5 12.5 5" /></svg>
